    ...
    <section>
      <div className="qci-card">
        ...
        {Object.keys(eng.savedKeys || {}).length > 0 && (
          <div className="qci-api-chips">
            {Object.keys(eng.savedKeys).map((name) => (
              <div key={name} className={`qci-api-chip${eng.savedKeys[name] === eng.apiKey.trim() ? ' active' : ''}`}>
        ...
        <div className="qci-actions-bar">
          <div />
          <button className="qci-btn" onClick={() => eng.setStep(2)}>
            {t({ ar: 'التالي: ملف العملاء ←', en: "Next: customers' file →" })}
          </button>
        </div>
      </div>
    </section>
