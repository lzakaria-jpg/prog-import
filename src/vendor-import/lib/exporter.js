/**
 * exporter.js — كتابة صفوف الموردين الجاهزة داخل نسخة من قالب Qoyod الرسمي
 * (vendor_import_template.xlsx)، ابتداءً من الصف الثاني (الصف الأول عناوين
...
/**
 * يكتب صفوف الموردين داخل نسخة من القالب المرفق بالأداة (ArrayBuffer)، ويُعيد Blob جاهزاً للتنزيل.
 * @param {Array<object>} rows صفوف buildRows (بعد validateAll)
 * @param {ArrayBuffer} templateArrayBuffer محتوى vendor_import_template.xlsx كما هو مُرفَق بالأداة
 */
export function exportContacts(rows, templateArrayBuffer) {
  ...
  // [ملاحظة] القالب الأصلي يحمل نطاقاً ضخماً جداً (أثر شائع لجلسة حفظ إكسل
  // سابقة، لا علاقة له بأعمدة القالب الفعلية) — نطاق الإخراج هنا يُضبط صراحةً
  // على عدد أعمدة الحقول الفعلي فقط (DEFAULT_KEYS.length، 15 لقالب الموردين).
