  ...
  return (
    <section>
      <div className="qvi-card">
        ...
        <p className="hint">
          {t({
            ar: 'التصدير يكتب داخل قالب Qoyod الرسمي (Import Vendors) ابتداءً من الصف الثاني — ملف تعيد رفعه بنفسك لاحقاً بواجهة قيود. الإرسال المباشر ينشئ/يحدّث كل صف فوراً عبر API.',
            en: "Export writes into Qoyod's official template (Import Vendors) starting from row 2 — a file you upload yourself later in Qoyod's UI. Direct send creates/updates each row immediately via the API.",
          })}
        </p>

        <div className={`qvi-msg ${bad.length ? 'warn' : 'ok'}`}>
        ...
        <div className="qvi-actions">
          <button className="qvi-btn dark" disabled={!eng.rows.length} onClick={() => eng.doExport('all')}>{t({ ar: 'تحميل الملف كاملاً', en: 'Download the full file' })}</button>
          <button className="qvi-btn" disabled={!good.length} onClick={() => eng.doExport('valid')}>{t({ ar: 'تحميل الصفوف السليمة فقط', en: 'Download valid rows only' })}</button>
          <button className="qvi-btn ghost" disabled={!bad.length} onClick={() => eng.doExport('errors')}>{t({ ar: 'تحميل تقرير الأخطاء', en: 'Download the error report' })}</button>
          {eng.canSendViaApi && (
            <button className="qvi-btn go"
        ...
        {!showSendModal && (eng.apiSending || eng.apiSendResult) && (
          <div className="qvi-btn" style={{ position: 'fixed', bottom: 20, insetInlineStart: 20, zIndex: 1001, borderRadius: 999, display: 'flex', alignItems: 'center', gap: 8, boxShadow: '0 12px 32px rgba(15,23,42,.25)' }}>
            <span style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }} onClick={() => setShowSendModal(true)}>
              ...
              {!eng.apiSending && eng.apiSendResult?.failed > 0 && (
                <span style={{ background: 'var(--qvi-err)', color: '#fff', borderRadius: 999, minWidth: 18, height: 18, fontSize: 10, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '0 4px' }}>{eng.apiSendResult.failed}</span>
              )}
            </span>
            {eng.apiSending && (
        ...
        <div className="qvi-msg info">
          {t({
            ar: 'الرقم المرجعي (Ref. No.) يُكتب في ملف القالب المُصدَّر كالمعتاد، لكنه لا يُرسَل إطلاقاً عبر الإرسال المباشر — لا حقل موثَّق له بواجهة Qoyod البرمجية للموردين. راجع الملاحظة بخطوة الربط لتفاصيل أوسع.',
            en: "The Ref. No. is written into the exported template file as usual, but is never sent via direct send — there's no documented field for it in Qoyod's vendor API. See the note on the mapping step for more.",
          })}
        </div>

        <div className="qvi-scroll">
