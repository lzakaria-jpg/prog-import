...
/** شاشة نتائج الإرسال المباشر للموردين عبر Qoyod API — تُفتح من Step4Export.jsx فقط */
export default function ApiSendResultsModal({ eng, onClose }) {
  ...
  return (
    <div className="qvi-modal-overlay" role="dialog" aria-modal="true" onClick={() => { if (!apiSending) onClose(); }}>
      <div className="qvi-modal wide" onClick={(e) => e.stopPropagation()}>
        <h3>
          {apiSending ? '⏳ ' : apiSendResult?.fatalError ? '⛔ ' : (failedCount > 0 ? '⚠️ ' : '✅ ')}
          {t({ ar: 'الإرسال المباشر للموردين عبر API', en: 'Direct vendor send via API' })}
        </h3>

        {apiSending && (
          <div className="qvi-send-progress">
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button type="button" className="qvi-btn ghost" onClick={onClose} title={t({ ar: 'تصغير — الإرسال يستمر بالخلفية', en: 'Minimize — sending continues in the background' })}>
                − {t({ ar: 'تصغير', en: 'Minimize' })}
              </button>
              <button type="button" className="qvi-btn danger" onClick={stopApiSend}>{t({ ar: 'إيقاف', en: 'Stop' })}</button>
            </div>
          </div>
        )}

        {!apiSending && apiSendResult?.fatalError && (
          <div className="qvi-msg err" style={{ marginBottom: 12 }}>⛔ {apiSendResult.fatalError}</div>
        )}

        {!apiSending && !apiSendResult?.fatalError && apiSendResult?.stoppedEarly && (
          <div className="qvi-msg warn" style={{ marginBottom: 12 }}>
        ...
        <div className="qvi-modal-scroll">
          {entries.length > 0 && (
            <div className="qvi-send-tabs">
              <button type="button" className={`qvi-send-tab${tab === 'all' ? ' active' : ''}`} onClick={() => setTab('all')}>
                <span className="n">{totalCount || entries.length}</span>
                <span className="l">{t({ ar: 'الكل', en: 'All' })}</span>
              </button>
              <button type="button" className={`qvi-send-tab${tab === 'success' ? ' active' : ''}`} onClick={() => setTab('success')}>
                <span className="n" style={{ color: 'var(--qvi-ok)' }}>{sentCount}</span>
                <span className="l">{t({ ar: 'نجح', en: 'Success' })}</span>
              </button>
              <button type="button" className={`qvi-send-tab${tab === 'error' ? ' active' : ''}`} onClick={() => setTab('error')}>
                <span className="n" style={{ color: 'var(--qvi-err)' }}>{failedCount}</span>
          ...
          {!apiSending && entries.length === 0 && !apiSendResult?.fatalError && (
            <div className="qvi-msg">{t({ ar: 'لا توجد نتائج بعد.', en: 'No results yet.' })}</div>
          )}
        </div>

        <div className="qvi-modal-actions">
          {!apiSending && apiSendResult && !apiSendResult.fatalError && (
            <button type="button" className="qvi-btn ghost" onClick={downloadApiSendResults}>
              ⬇️ {t({ ar: 'تحميل تقرير النتائج (Excel)', en: 'Download results report (Excel)' })}
            </button>
          )}
          <button type="button" className="qvi-btn ghost" disabled={apiSending} onClick={onClose}>{t({ ar: 'إغلاق', en: 'Close' })}</button>
        </div>
      </div>
    </div>
  );
