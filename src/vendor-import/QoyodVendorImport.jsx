/**
 * QoyodVendorImport — المكون الرئيسي لأداة استيراد الموردين.
 * نفس بنية src/customer-import/QoyodCustomerImport.jsx حرفياً (وهي بدورها
 * تتبع عقد src/bill-import/QoyodBillImport.jsx) — الفرق الوحيد هنا: يعمل على
 * /vendors بدل /customers، وقالبه بلا أعمدة عنوان الشحن (راجع lib/fields.js).
 *
...
const QoyodVendorImport = forwardRef(function QoyodVendorImport({
  apiKey = '',
  showHeader = true,
  className = '',
  onExport,
      ...
      <div className="qvi-wrap">
        ...
        <details className="qvi-guide">
          ...
          <ul>
            ...
            <li><b>{t({ ar: 'التكرار بالاسم:', en: 'Name duplicates:' })}</b> {t({ ar: 'تحذير عند تطابق أو تشابه كبير مع مورد موجود فعلاً — يتطلب اختياراً صريحاً (إنشاء/تحديث/تجاوز) قبل الإرسال المباشر.', en: 'A warning on an exact or highly similar match with an existing vendor — requires an explicit choice (create/update/skip) before direct send.' })}</li>
            <li><b>{t({ ar: 'الرقم المرجعي:', en: 'Ref. No.:' })}</b> {t({ ar: 'غير موثَّق بواجهة قيود البرمجية للموردين — مرجع محلي بالأداة/ملف التصدير فقط، لا يُرسَل عبر API.', en: "Not documented in Qoyod's vendor API — a local, tool/export-file-only reference, never sent via the API." })}</li>
          </ul>
        </details>
      </div>
    </div>
...
export default QoyodVendorImport;
