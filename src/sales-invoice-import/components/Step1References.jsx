...
export default function Step1References({ state, actions }) {
  ...
  const missingRequired = [
    !template && 'قالب قيود',
    !(references.customers?.length > 0) && 'ملف العملاء',
    !(references.products?.length > 0) && 'ملف المنتجات',
    !(references.locationStock?.rows?.length) && 'ملف كميات المنتجات حسب المواقع',
  ].filter(Boolean);
  ...
  return (
    <>
      ...
      <p className="qii-page-sub">
        ارفع قالب قيود وملفات العملاء والمنتجات وكميات المواقع. الأداة تبني كل عمليات التحقق على هذه الملفات، لا على قيم مخزّنة مسبقاً.
      </p>

      {missingRequired.length > 0 && (
        <Note tone="stop">
          <strong>لا يمكن المتابعة إلى ملف العميل قبل رفع الملفات الإلزامية التالية:</strong> {missingRequired.join(' · ')}.
          كل الملفات الأربعة إلزامية — ملف كميات المنتجات حسب المواقع تحديداً هو المصدر الوحيد المعتمد للكمية المتاحة
          لكل منتج في كل موقع، ولا يُستعاض عنه بإجمالي الكمية في ملف المنتجات.
        </Note>
      )}
      ...
      >
        <Note>
          يُستخدم لتعريف المنتجات وأسمائها ومراجعها: التأكد أن كل منتج في ملف العميل موجود فعلاً في قيود.
          أي إجمالي كمية في هذا الملف <strong>معلوماتي فقط ولا يُستخدم للتحقق من الكمية المتاحة لكل موقع</strong> —
          فحص الكميات يعتمد حصراً على ملف كميات المنتجات حسب المواقع أدناه.
        </Note>

        <FileDrop
          label="اسحب ملف المنتجات هنا"
          hint="xlsx أو csv — يحتاج الرمز أو الباركود، والاسم"
          accept=".xlsx,.csv"
          file={productsFile}
        ...
        {references.products?.length > 0 && (
          <>
            ...
            {!locationStockFile && (
              <Note tone="stop">
                لا يوجد عمود كمية متاحة في هذا الملف، وقوالب رفع المنتجات في قيود لا تحمله أصلاً — وهذا متوقّع؛
                إجمالي هذا الملف لا يُستخدم للتحقق أصلاً. <strong>ارفع ملف كميات المنتجات حسب المواقع أدناه</strong>
                لتفعيل فحص الكميات، فهو إلزامي ولا يمكن المتابعة بدونه.
              </Note>
            )}
          </>
        )}
      ...
      <Card
        title="ملف كميات المنتجات حسب المواقع"
        aside={locationStockInfo ? <Badge tone="ok">{i(locationStockInfo.count)} منتج</Badge> : <Badge tone="stop">مطلوب</Badge>}
      >
        <Note tone={locationStockInfo ? undefined : 'stop'}>
          <strong>إلزامي، وهو المصدر الوحيد المعتمد للكمية المتاحة لكل منتج في كل موقع</strong> — لا إجمالي الكمية في
          ملف المنتجات. يحدد الكمية المتوفرة من كل منتج في كل موقع — عمود أو أكثر لتعريف المنتج (الرقم التسلسلي/الباركود/الاسم)،
          وبقية الأعمدة كل واحد منها موقع. أعمدة المواقع تُكتشف تلقائياً من رؤوس الملف. الفحص يقارن كمية كل فاتورة
          برصيد موقعها تحديداً، ويُحدَّث هذا الرصيد تسلسلياً مع كل فاتورة تُقبل — لا برصيد أصلي ثابت يتكرر لكل فاتورة.
          منتج غير موجود في هذا الملف لا يُعتبر خطأً؛ هو منتج غير مخزَّن يُباع بلا حد.
        </Note>
