...
export const AUX_FIELD_KEYWORDS = {
  ...
  // [إضافة] عمود "فئة المنتج" — اسم الفئة كما كُتب حرفيًا بملف العميل، يُخزَّن خامًا
  // بلا أي مطابقة هنا (نفس نمط _project تمامًا) على row.categoryRef. يُستخدَم حصريًا
  // عند إنشاء منتج غير موجود تلقائيًا (لوحة مراجعة الكيانات الناقصة بالخطوة 4) —
  // لا علاقة له بأي عمود من أعمدة القالب الرسمي A-V، ولا يؤثر على أي منتج موجود
  // فعلًا (فئة المنتجات الموجودة لا تُعدَّل من هذه الأداة إطلاقًا).
  _category: ['فئة المنتج','الفئة','فئة الصنف','تصنيف المنتج','category','product category','item category'],
  // [إضافة] عمود "وحدة المنتج الأساسية" — يُخزَّن خامًا على row.unitRef، نفس نمط
  // _category أعلاه تمامًا. [تمييز مهم] هذا مختلف كليًا عن عمود القالب الرسمي Q
  // ("وحدة التحويل" بـCOLUMNS) — Q هي وحدة البيع/التحويل لبند فاتورة **منتج موجود
  // أصلًا** (مثل "كرتون" فوق وحدة أساسية "قطعة")، بينما _unit هنا هي الوحدة
  // الأساسية للمنتج نفسه (product_unit_type_id) وتُستخدَم فقط عند إنشاء منتج غير
  // موجود من الصفر. لا تُخلَط الاثنتان بأي كود أو تعليق آخر بهذه الميزة.
  _unit: ['وحدة المنتج','الوحدة الأساسية','وحدة الصنف الأساسية','unit type','base unit','product unit','unit of measure'],
  // [إضافة] دعم سندات القبض المرتبطة بفواتير ضمن نفس ملف العميل الخام (طلب
  // صريح من المستخدم 2026-09-16) — راجع تعليق رأس engine/receipts.js للتفصيل
  // الكامل. ثلاثة حقول خام تُخزَّن بلا أي مطابقة هنا (نفس نمط _project/_category
  // أعلاه تمامًا): _docType يميّز صف "فاتورة" عن صف "سند قبض" (row.docType)،
  // _paymentAmount مبلغ السند (row.paymentAmount)، _paymentAccountCode كود حساب
  // الدفع كما كُتب بالملف (row.paymentAccountCode) — يُطابَق بكود حساب حقيقي
  // (GET /accounts) لاحقًا فقط عبر لوحة مراجعة مخصَّصة قبل الإرسال الفعلي.
  _docType: ['النوع','نوع المستند','نوع السطر','document type','row type','entry type'],
  _paymentAmount: ['قيمة الدفعة','مبلغ الدفعة','قيمة السند','مبلغ السند','payment amount','receipt amount','amount paid'],
  _paymentAccountCode: ['حساب الدفع','كود حساب الدفع','رمز حساب الدفع','payment account','account code'],
};
...
export const AUX_FIELD_LABELS = {
  ...
  _category: {icon:'🏷️', label:'فئة المنتج', hint:'(اختياري — تُستخدَم فقط عند إنشاء منتج غير موجود تلقائيًا؛ بلا هذا العمود يختار المستخدم فئة افتراضية واحدة للدفعة كاملة)'},
  _unit: {icon:'📏', label:'وحدة المنتج الأساسية', hint:'(اختياري — الوحدة الأساسية للمنتج عند إنشائه من الصفر، مختلفة عن عمود "وحدة التحويل" Q؛ بلا هذا العمود يختار المستخدم وحدة افتراضية واحدة للدفعة كاملة)'},
  _docType: {icon:'🧾', label:'النوع (فاتورة / سند قبض)', hint:'(اختياري — بلا هذا العمود، كل الصفوف تُعامَل كفواتير كما كان دومًا؛ صف بقيمة "سند قبض" يُنشئ دفعة مرتبطة بفاتورة نفس المرجع عند الإرسال عبر API)'},
  _paymentAmount: {icon:'💰', label:'قيمة الدفعة (لصفوف سند القبض فقط)', hint:'(إلزامي لأي صف "سند قبض" — مبلغ السند نفسه)'},
  _paymentAccountCode: {icon:'🏦', label:'حساب الدفع (لصفوف سند القبض فقط)', hint:'(إلزامي لأي صف "سند قبض" — كود الحساب النقدي/البنكي المستلِم، يُطابَق بدليل حسابات المنشأة قبل الإرسال)'},
};
