...
/**
 * [إضافة] شاشة نتائج الإرسال المباشر لفواتير المشتريات عبر Qoyod API — تُفتح
 * من Step4Export.jsx فقط، بعد ضغط "إرسال عبر API". نفس فكرة الشاشة المكافئة
 * بأداة فواتير المبيعات (ApiSendResultsModal.jsx هناك)، بهوية qbi-* الخاصة
 * بهذه الأداة. لا تُعدِّل أي حالة تخص المطابقة أو التصدير اليدوي.
 */
export default function ApiSendResultsModal({ eng, onClose }) {
  ...
  return (
    <div className="qbi-modal-overlay" role="dialog" aria-modal="true" onClick={() => { if (!apiSending) onClose(); }}>
      <div className="qbi-modal wide" onClick={(e) => e.stopPropagation()}>
        <h3>
          {apiSending ? '⏳ ' : apiSendResult?.fatalError ? '⛔ ' : (failedCount > 0 ? '⚠️ ' : '✅ ')}
          {t({ ar: 'الإرسال المباشر لفواتير المشتريات عبر API', en: 'Direct purchase bill send via API' })}
        </h3>
        ...
        {!apiSending && apiSendResult?.fatalError && (
          <div className="qbi-msg err" style={{ marginBottom: 12 }}>⛔ {apiSendResult.fatalError}</div>
        )}

        {!apiSending && !apiSendResult?.fatalError && apiSendResult?.stoppedEarly && (
          <div className="qbi-msg warn" style={{ marginBottom: 12 }}>
        ...
        <div className="qbi-modal-scroll">
          {entries.length > 0 && (
            <div className="qbi-send-tabs">
              <button type="button" className={`qbi-send-tab${tab === 'all' ? ' active' : ''}`} onClick={() => setTab('all')}>
                <span className="n">{totalCount || entries.length}</span>
                <span className="l">{t({ ar: 'الكل', en: 'All' })}</span>
              </button>
              <button type="button" className={`qbi-send-tab${tab === 'success' ? ' active' : ''}`} onClick={() => setTab('success')}>
                <span className="n" style={{ color: 'var(--qbi-ok)' }}>{sentCount}</span>
                <span className="l">{t({ ar: 'نجح', en: 'Success' })}</span>
              </button>
              <button type="button" className={`qbi-send-tab${tab === 'error' ? ' active' : ''}`} onClick={() => setTab('error')}>
                <span className="n" style={{ color: 'var(--qbi-err)' }}>{failedCount}</span>
          ...
          {visibleEntries.length > 0 && (
            <table>
              <thead>
                <tr>
                  <th>{t({ ar: 'مرجع الفاتورة', en: 'Bill reference' })}</th>
          ...
          {!apiSending && entries.length === 0 && !apiSendResult?.fatalError && (
            <div className="qbi-msg">{t({ ar: 'لا توجد نتائج بعد.', en: 'No results yet.' })}</div>
          )}
        </div>

        <div className="qbi-modal-actions">
          {!apiSending && apiSendResult && !apiSendResult.fatalError && (
            <button type="button" className="qbi-btn ghost" onClick={downloadApiSendResults}>
              ⬇️ {t({ ar: 'تحميل تقرير النتائج (Excel)', en: 'Download results report (Excel)' })}
            </button>
          )}
          <button type="button" className="qbi-btn ghost" disabled={apiSending} onClick={onClose}>{t({ ar: 'إغلاق', en: 'Close' })}</button>
        </div>
      </div>
    </div>
  );
