...
describe("لوحة تحديد الأعمدة يدويًا — المشروع والموقع", () => {
  ...
  // [تصحيح 2026-09-15] بلاغ مستخدم حي: كانت قيمة عمود "مستوى السطر" تُرفَع
  // خطأً لتصبح افتراضي القيد أيضًا حين لا يوجد عمود "مستوى قيد" منفصل - فتُرسَل
  // القيمة مرتين. افتراضي القيد الآن يبقى فارغًا ما لم يُكتب صراحةً بعمود
  // "مستوى قيد" مخصَّص - القيمة بعمود "مستوى السطر" تبقى خاصة بسطرها فقط.
  it("بلا عمود مستوى قيد منفصل: افتراضي القيد يبقى فارغًا (لا يُشتَق من عمود مستوى السطر إطلاقًا)", () => {
    const rows = [
    ...
    expect(entry.rows[1].location).toBe("الفرع الرئيسي"); // يبقى خاصًا بسطره فقط
  });
});

describe("التعرّف التلقائي (بلا تدخل يدوي) على أعمدة المشروع/الموقع", () => {
  it("قالب الأداة (Schema A - 'تسلسل القيد') يقرأ عمود مشروع واحد كمستوى سطر - وافتراضي القيد يبقى فارغًا (لا عمود قيد منفصل بالملف)", () => {
    const rows = [
    ...
    expect(entry.rows[1].project).toBe("");
    expect(entry.project).toBe(""); // [تصحيح 2026-09-15] لا يُشتَق من عمود مستوى السطر
  });
  ...
  it("جدول عربي مرن (Schema E - 'رقم العملية') يقرأ عمود الموقع تلقائيًا كمستوى سطر - وافتراضي القيد يبقى فارغًا", () => {
    const rows = [
    ...
    expect(entry.rows[1].location).toBe("");
    expect(entry.location).toBe(""); // [تصحيح 2026-09-15] لا يُشتَق من عمود مستوى السطر
  });
