...
async function onRequestPost(context2) {
  const apiKey = context2.env && context2.env.ANTHROPIC_API_KEY;
  if (!apiKey) {
    return new Response(
...
async function onRequest(context2) {
  if (context2.request.method === "POST") return onRequestPost(context2);
  if (context2.request.method === "OPTIONS") return onRequestOptions();
...
// ../.wrangler/tmp/pages-XvRAs4/functionsRoutes-0.509323758381008.mjs
var routes = [
  {
